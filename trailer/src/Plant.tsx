...
// Ported from frontend/app/components/shared/VirtualPlant.tsx — same path
// data, but sway is driven by useCurrentFrame() instead of motion/react's
// wall-clock animation, since Remotion needs deterministic per-frame render.
export type PlantStage = "seed" | "sprout" | "sapling" | "young_tree" | "mature_tree" | "blooming";
...
const SWAY: Record<PlantStage, number> = {
  seed: 0,
  sprout: 1,
  sapling: 1.8,
  young_tree: 1.8,
...
export const Plant: React.FC<{ stage: PlantStage; size?: number }> = ({ stage, size = 220 }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const Glyph = STAGES[stage];
  const swayAmount = SWAY[stage];
  const rotate = swayAmount > 0 ? Math.sin((frame / fps) * (Math.PI * 2 / 5)) * swayAmount : 0;

  return (
    <svg viewBox="0 0 120 120" width={size} height={size} role="img">
      {GROUND}
      <g style={{ transformOrigin: "60px 100px", transform: `rotate(${rotate}deg)` }}>
        <Glyph />
      </g>
    </svg>
  );
