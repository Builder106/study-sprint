...
export function TimerCard({ onLogSession }: Props) {
  ...
  const handlePhaseComplete = () => {
    ...
    if (current === 'work') {
      ...
      const nextPhase: PomodoroPhase = nextCycles % LONG_BREAK_EVERY === 0
        ? 'long_break'
        : 'short_break';
      setPhase(nextPhase);
  ...
  const display = mode === 'stopwatch' ? formatClock(elapsed) : formatClock(remaining);

  const suggestedMinutes = mode === 'stopwatch' ? Math.max(0, Math.floor(elapsed / 60)) : 0;
